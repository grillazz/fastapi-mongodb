...
async def retrieve_document(document_id: str, collection: str) -> dict:
    ...
    if document := app_name.app.database[collection].find_one(document_filter):
...
async def create_document(document: dict, collection: str) -> dict:
    ...
    """
    try:
        document = app_name.app.database[collection].insert_one(document)
...
async def get_mongo_meta() -> dict:
    list_databases = await app_name.app.mongo_client.list_database_names()
    list_of_collections = {}
    for db in list_databases:
        list_of_collections[db] = await app_name.app.mongo_client[
            db
        ].list_collection_names()
    mongo_meta = await app_name.app.mongo_client.server_info()
    return {
